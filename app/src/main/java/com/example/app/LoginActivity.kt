                ...
                .enqueue(object: Callback<LoginResponse>{
                    ...
                    override fun onResponse(call: Call<LoginResponse>, response: Response<LoginResponse>) {
                        if(response.body()?.header?.code == 200){
                            print(response.body())
                            println("on response *********************************************************************")
                            SharedPrefManager.getInstance(applicationContext).saveUser(response.body()?.response!!)
                            ...
                            Log.d("500", "error !!! ")
                            Toast.makeText(applicationContext, response.body()?.header!!.message, Toast.LENGTH_LONG).show()
                        }
